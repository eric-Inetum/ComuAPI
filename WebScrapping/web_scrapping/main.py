...
def peticionAPI(i, player_id, id_jugador, nombre_jugador, propietario, nombre_equipo, posicion, titular, partidos_jugados, ranking_general, ranking_equipo
                , ranking_posicion, mejorFichaje, media_sofascore, media_puntos, total_puntos, puntos_buenos, ofertaMin, precio_jugador, precio_min
    ...
    urlAPI = "http://10.228.64.253:3002/api/v1/jugadores"
    ...
    if responseAPI.status_code == 200:
        ...
        api_id = []
        jugadores_api = data["jugadores"]
        for j in jugadores_api:
            api_id.append(str(j['id_jugador']))
        
        datosJson = {
            
            "id": int(id_jugador),
            "nombre": f"{nombre_jugador}",
            "propietario": "prueba final",#f"{propietario}",
        ...
        if str(id) in api_id:
            ...
            patch_response = requests.patch(urlAPI + f"/{id}/actualizar", json=datosJson)

            if patch_response.status_code == 200:
                print(f"Patch realizado a jugador {nombre_jugador}")
            else:
                print(f"Patch no realizado a jugador {nombre_jugador}")

        else:
            post_response = requests.post(urlAPI + "/insertar", json=datosJson)
...
while i <= cont_lineas:
    ...
    if response.status_code == 200:
        ...
        # En esta variable se pretendía guardar la suma de los puntos de cada jornada que supere el numero 5, la cosa es que al intentar recoger los puntos
        # por culpa de la estructura del html era imposible acceder a estos despúes de haberlo intentado de diversas formas. Por temas de organización de la
        # base de datos no se podía quitar este campo así que siempre será null.    
        puntos_buenos = None
        

        # Una vez recogidos todos los datos para almacenar en formato json se llama a la función
        peticionAPI(i, player_id, id_jugador, nombre_jugador, propietario, nombre_equipo, posicion, titular, partidos_jugados, ranking_general, ranking_equipo
        , ranking_posicion, mejorFichaje, media_sofascore, media_puntos, total_puntos, puntos_buenos, ofertaMin, precio_jugador, precio_min, precio_max
